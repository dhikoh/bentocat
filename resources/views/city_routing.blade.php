...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-16">
    ...
    <div class="text-center space-y-6 max-w-3xl mx-auto">
        <span class="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-700 text-xs font-extrabold px-4 py-1.5 rounded-full uppercase tracking-wider">
    ...
    <div class="space-y-8">
        ...
        <div class="space-y-4">
            @forelse($outlets as $outlet)
                <div class="bg-white border {{ !$outlet->is_mitra ? 'border-slate-100 opacity-90 shadow-sm' : ($outlet->featured ? 'border-amber-500/80 shadow-md shadow-amber-500/5 bg-amber-50/15' : 'border-slate-100 shadow-sm') }} p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md hover:border-slate-200/80 transition-all duration-300">
                    <div class="space-y-3">
                        <div class="flex flex-wrap items-center gap-2.5">
                            ...
                            @if($outlet->is_mitra)
                                @if($outlet->featured)
                                    <span class="inline-flex items-center gap-1 bg-amber-500/10 border border-amber-500/20 text-amber-700 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                        ⭐ Rekomendasi
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 bg-green-500/10 border border-green-500/20 text-green-700 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                                        ✓ Mitra Aktif
                                    </span>
                                @endif
                            @endif
                        ...
                        <div class="flex items-start gap-2 max-w-2xl text-slate-600">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="inline-flex items-center gap-1.5 bg-slate-50 border border-slate-200/60 text-slate-600 px-3 py-1 rounded-xl text-xs font-medium">
                    ...
                    <div class="shrink-0 flex items-center justify-end w-full md:w-auto">
                        <a href="{{ $waUrl }}" target="_blank" class="w-full md:w-auto bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold px-6 py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 select-none">
            ...
            @empty
                ...
                <div class="bg-white border border-slate-100 p-12 rounded-3xl text-center shadow-md space-y-6 max-w-xl mx-auto">
                    ...
                    @if($distributor)
                        <div class="bg-slate-50 max-w-md mx-auto p-6 rounded-2xl border border-slate-200/60 text-left space-y-4 shadow-sm">
                            <span class="block text-[9px] font-extrabold text-slate-450 uppercase tracking-widest">Distributor Regional:</span>
                            <div class="text-xs space-y-1.5">
                                ...
                                <span class="block text-slate-650">Alamat: {{ $distributor->alamat }}</span>
                            </div>
                            @php
                            ...
                            @endphp
                            <a href="{{ $waDistUrl }}" target="_blank" class="w-full bg-white border border-slate-200 hover:border-amber-500/30 hover:text-amber-700 text-slate-700 text-xs font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm">
    ...
    <div class="border-t border-slate-200 pt-12 text-center space-y-4">
        <h3 class="font-outfit font-bold text-lg text-slate-900">Ingin koordinat Anda disinkronkan secara presisi?</h3>
        <p class="text-xs text-slate-655 max-w-sm mx-auto leading-relaxed">Gunakan formulir pencarian interaktif di halaman utama untuk menghitung rute kurir dan melacak jarak km terdekat.</p>
        <a href="{{ route('home') }}#cari-outlet" class="inline-block bg-white hover:bg-slate-50 border border-slate-200 text-amber-700 hover:text-amber-800 font-bold text-xs px-6 py-3.5 rounded-xl shadow-sm transition-all">
