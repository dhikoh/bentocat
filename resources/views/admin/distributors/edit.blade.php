...
@section('title', 'Edit Distributor - ' . $distributor->nama)
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Distributor: {{ $distributor->nama }}</h1>
        <p class="text-sm text-slate-400">Perbarui informasi kontak, alamat gudang, atau status aktif distributor.</p>
    </div>

    <div class="bg-slate-900/40 border border-slate-800/80 p-6 md:p-8 rounded-3xl backdrop-blur-md">
        <form action="{{ route('admin.distributors.update', $distributor->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="nama" class="block text-xs font-bold text-slate-400 uppercase mb-2">Nama Distributor</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama', $distributor->nama) }}" required 
                ...
                <div>
                    ...
                    <select name="provinsi_id" id="provinsi_id" required 
                        ...
                        @foreach($provinces as $province)
                            <option value="{{ $province->id }}" {{ old('provinsi_id', $distributor->city ? $distributor->city->provinsi_id : null) == $province->id ? 'selected' : '' }}>
                                {{ $province->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="kota_id" id="kota_id" required 
                        ...
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}" {{ old('kota_id', $distributor->kota_id) == $city->id ? 'selected' : '' }}>
                                {{ $city->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="pic" id="pic" value="{{ old('pic', $distributor->pic) }}" required 
                ...
                <div>
                    ...
                    <input type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $distributor->whatsapp) }}" required 
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="4" required 
                          ...
                          class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">{{ old('alamat', $distributor->alamat) }}</textarea>
            </div>
            ...
            <div class="border-t border-slate-800/80 pt-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 text-sm text-slate-300 select-none cursor-pointer">
                        <input type="checkbox" name="tampil_ke_publik" value="1" {{ $distributor->tampil_ke_publik ? 'checked' : '' }} class="rounded border-slate-850 text-amber-500 bg-slate-950">
                        Tampilkan Publik
                    </label>
                    ...
                    <label class="flex items-center gap-2 text-sm text-slate-300 select-none cursor-pointer">
                        ...
                        <select name="status" class="bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 text-xs text-slate-200">
                            <option value="ACTIVE" {{ $distributor->status === 'ACTIVE' ? 'selected' : '' }}>ACTIVE / AKTIF</option>
                            <option value="INACTIVE" {{ $distributor->status === 'INACTIVE' ? 'selected' : '' }}>INACTIVE / NONAKTIF</option>
                        </select>
                    </label>
                </div>
