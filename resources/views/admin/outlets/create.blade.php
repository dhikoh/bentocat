...
@section('title', 'Tambah Outlet/Petshop')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-white">Tambah Outlet / Petshop Baru</h1>
        <p class="text-sm text-slate-400">Daftarkan mitra toko fisik (petshop) terdekat untuk melayani pembelian customer lokal.</p>
    </div>
        ...
        <form action="{{ route('admin.outlets.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nama_outlet" id="nama_outlet" value="{{ old('nama_outlet') }}" required 
            ...
            <div>
                ...
                <textarea name="alamat_lengkap" id="alamat_lengkap" rows="3" required 
                          ...
                          class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">{{ old('alamat_lengkap') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="url" name="google_maps_url" id="google_maps_url" value="{{ old('google_maps_url') }}" 
            ...
            <div class="border-t border-slate-800/80 pt-6">
                ...
                <select name="delivery_mode" id="delivery_mode" required onchange="handleDeliveryModeChange()"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">
                    <option value="SELF_DELIVERY" {{ old('delivery_mode') === 'SELF_DELIVERY' ? 'selected' : '' }}>🛵 Pengiriman Mandiri oleh Outlet (Self-Delivery)</option>
                    <option value="RECOMMENDED_SHIPPING_CONTACT" {{ old('delivery_mode') === 'RECOMMENDED_SHIPPING_CONTACT' ? 'selected' : '' }}>📋 Menggunakan Kontak Kurir/Ekspedisi Rekomendasi</option>
                    <option value="PICKUP_ONLY" {{ old('delivery_mode') === 'PICKUP_ONLY' ? 'selected' : '' }}>🚶 Hanya Ambil di Tempat (Pickup Only)</option>
                </select>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($shippingContacts as $contact)
                        <label class="flex items-start gap-3 bg-slate-900/60 p-3 rounded-xl border border-slate-800 cursor-pointer select-none hover:border-slate-700">
                            <input type="checkbox" name="shipping_contacts[]" value="{{ $contact->id }}" class="mt-0.5 rounded border-slate-850 text-amber-500 bg-slate-950">
                            <div>
            ...
            <div class="border-t border-slate-800/80 pt-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 text-sm text-slate-300 select-none cursor-pointer">
                        <input type="checkbox" name="featured" value="1" class="rounded border-slate-850 text-amber-500 bg-slate-950">
                    ...
                    <label class="flex items-center gap-2 text-sm text-slate-300 select-none cursor-pointer">
                        <input type="checkbox" name="is_hidden" value="1" class="rounded border-slate-850 text-amber-500 bg-slate-950">
...
<script>
    ...
    // Call on load
    document.addEventListener('DOMContentLoaded', handleDeliveryModeChange);
</script>
@endsection
