...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-400">Daftar mitra toko retail (petshop) yang menyediakan stok BentoCat di tingkat kota.</p>
        </div>
        <a href="{{ route('admin.outlets.create') }}" class="bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold px-5 py-2.5 rounded-xl shadow-lg shadow-amber-500/10 transition-all flex items-center gap-2">
    ...
    <div class="bg-slate-900/40 border border-slate-800/80 p-4 rounded-2xl">
        <form action="{{ route('admin.outlets.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-300">
                ...
                <tbody class="divide-y divide-slate-850">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-slate-500 italic">Tidak ditemukan outlet/petshop terdaftar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
