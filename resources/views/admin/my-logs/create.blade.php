        ...
        <form action="{{ route('admin.my-logs.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-800 pt-6">
                <div class="space-y-2">
                    ...
                    <select name="crm_stage" id="crm_stage" class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-3 text-sm text-slate-200 focus:outline-none transition-all">
                        <option value="Cold" {{ old('crm_stage', 'Cold') == 'Cold' ? 'selected' : '' }}>❄️ Cold (Dingin)</option>
                        <option value="Warm" {{ old('crm_stage') == 'Warm' ? 'selected' : '' }}>🔥 Warm (Hangat)</option>
                        <option value="Hot" {{ old('crm_stage') == 'Hot' ? 'selected' : '' }}>⚡ Hot (Sangat Potensial)</option>
                        <option value="Closed-Won" {{ old('crm_stage') == 'Closed-Won' ? 'selected' : '' }}>🎉 Closed-Won (Berhasil)</option>
                        <option value="Closed-Lost" {{ old('crm_stage') == 'Closed-Lost' ? 'selected' : '' }}>❌ Closed-Lost (Gagal/Batal)</option>
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-4 py-2">
                        <input type="range" name="potential_closing" id="potential_closing" min="0" max="100" value="{{ old('potential_closing', 0) }}" 
            ...
            <div class="space-y-2">
                ...
                <textarea name="followup_feedback" id="followup_feedback" rows="3" placeholder="Tuliskan respon, feedback, tanggapan, atau negosiasi yang terjadi dari pihak pelanggan."
                          class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-3 text-sm text-slate-200 focus:outline-none transition-all">{{ old('followup_feedback') }}</textarea>
            </div>
