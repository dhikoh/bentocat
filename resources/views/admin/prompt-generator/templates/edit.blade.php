...
@section('title', 'Edit Template Prompt')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Template Prompt</h1>
        <p class="text-sm text-slate-400">Perbarui konfigurasi base prompt, kategori, atau variabel dinamis template.</p>
    </div>
    ...
    <div class="bg-slate-900/60 border border-slate-800 p-6 rounded-3xl backdrop-blur-md shadow-xl">
        <form action="{{ route('admin.prompt-generator.templates.update', $template->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $template->name) }}" required class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">
                </div>

                <!-- Category -->
                <div class="space-y-2">
                    <label for="category" class="block text-sm font-semibold text-slate-300">Kategori</label>
                    <input type="text" id="category" name="category" value="{{ old('category', $template->category) }}" required class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="target_audience" name="target_audience" value="{{ old('target_audience', $template->target_audience) }}" required class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">
                </div>

                <!-- Tone -->
                <div class="space-y-2">
                    <label for="tone" class="block text-sm font-semibold text-slate-300">Gaya Bahasa Default</label>
                    <input type="text" id="tone" name="tone" value="{{ old('tone', $template->tone) }}" required class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="placeholders" name="placeholders" value="{{ old('placeholders', $template->placeholders) }}" placeholder="Contoh: nama_petshop,harga,bonus_tambahan" class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">
            ...
            <div class="space-y-2">
                ...
                <textarea id="base_prompt" name="base_prompt" rows="8" required class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl p-4 text-sm text-slate-200 font-mono focus:outline-none transition-all">{{ old('base_prompt', $template->base_prompt) }}</textarea>
