...
@section('title', 'Edit Kontak Pengiriman - ' . $shippingContact->nama)
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Kontak Pengiriman: {{ $shippingContact->nama }}</h1>
        <p class="text-sm text-slate-400">Perbarui rincian kurir lokal, nomor WhatsApp aktif, jangkauan pengantaran, atau status kurir.</p>
    </div>

    <div class="bg-slate-900/40 border border-slate-800/80 p-6 md:p-8 rounded-3xl backdrop-blur-md">
        <form action="{{ route('admin.shipping-contacts.update', $shippingContact->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $shippingContact->nama) }}" required 
                ...
                <div>
                    ...
                    <input type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $shippingContact->whatsapp) }}" required 
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="4" 
                          ...
                          class="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:outline-none transition-all">{{ old('keterangan', $shippingContact->keterangan) }}</textarea>
            </div>
            ...
            <div class="border-t border-slate-800/80 pt-6 flex items-center justify-between">
                <label class="flex items-center gap-2 text-sm text-slate-300 select-none cursor-pointer">
                    <input type="checkbox" name="aktif" value="1" {{ $shippingContact->aktif ? 'checked' : '' }} class="rounded border-slate-850 text-amber-500 bg-slate-950">
